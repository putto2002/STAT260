...
```{r}
cpsdata = read.csv('../lab04/cps.csv')
str(cpsdata)
```
...
```{r}
ggplot(data=cpsdata, aes(x=as.factor(Union), y=Wage)) + geom_boxplot() +
  xlab("Union Membership (1=member, 0=non-member)") + ylab("Wages (US dollars/hour)") +
  ggtitle("Wage distributions for Union members and non-members")
```
...
```{r}
ggplot(data=cpsdata, aes(x=as.factor(Union), y=Wage)) + 
  geom_boxplot(outlier.alpha=0) + geom_jitter(width=0.15, alpha=0.2) +
  xlab("Union Membership (1=member, 0=non-member)") + ylab("Wages (US dollars/hour)") +
  ggtitle("Wage distributions for Union members and non-members")
```
...
```{r}
ggplot(data=cpsdata, aes(fill=as.factor(Union), x=Wage)) + 
  geom_density(alpha=0.3) +
  xlab("Wages (US dollars/hour)") +
  ggtitle("Wage distributions for Union members and non-members")
```
...
```{r}
ggplot(data=cpsdata, aes(x=Experience, y=Wage, colour=as.factor(Race))) +
  geom_point() + xlab("Experience (years)") + ylab("Wages (US dollars/hour)")
```
...
ggplot(data=cpsdata, aes(x=Experience, y=Wage, colour=as.factor(Race))) +
  geom_point() + xlab("Experience (years)") + ylab("Wages (US dollars/hour)") +
  scale_colour_discrete(labels=c("Other", "Hispanic", "White")) +
  labs(colour="Race")
```
