...
```{r}
data <- read.csv("./aki.csv", header = TRUE)
head(data)
...
data %>% ggplot(aes(x=age))+
  geom_histogram(bins = 15, fill = "wheat", col = "brown")+
  theme_dark()+
  xlab("Age (years)")+
...
```{r}
data %>% ggplot(aes(y=weight, x=factor(diab, label=c("Diabetic", "Non-diabetic"))))+
  geom_boxplot()+
  xlab("")+
  ggtitle("Boxplot of weight vs. diabetes")
